...
import { ChakraProvider, Box, Container, Flex, Heading, Button, IconButton, Text, Spinner, Alert, AlertIcon, useDisclosure, useToast, SimpleGrid, Stack, HStack } from '@chakra-ui/react';
...
const App = () => {
    ...
    const navButtons = [
      { mode: 'dashboard', icon: <LayoutGrid size={16} />, label: 'Dashboard' },
      { mode: 'byDay', icon: <Clock size={16} />, label: 'By Day' },
      { mode: 'biweekly', icon: <TrendingUp size={16} />, label: 'Bi-Weekly' },
      { mode: 'floor', icon: <TrendingUp size={16} />, label: 'Floor' },
                ...
                <Box minH="100vh" bg="gray.50">
                        <Box bg="teal.600" px={4} py={3} shadow="md">
                                  <Flex align="center" justify="space-between" maxW="1200px" mx="auto">
                                              ...
                                              <HStack>
                                                            <Button leftIcon={<Plus size={16} />} colorScheme="whiteAlpha" size="sm" onClick={() => { setEditingShift(null); setDialogOpen(true); }}>
                                                                            Add Shift
                                                            </Button>Button>
                        ...
                        <Box bg="white" borderBottom="1px" borderColor="gray.200" px={4}>
                                  <Flex maxW="1200px" mx="auto" gap={1} py={2} overflowX="auto">
                                    {navButtons.map(btn => (
                          <Button key={btn.mode} leftIcon={btn.icon} size="sm" variant={viewMode === btn.mode ? 'solid' : 'ghost'} colorScheme={viewMode === btn.mode ? 'teal' : 'gray'} onClick={() => setViewMode(btn.mode)}>
                            {btn.label}
                          </Button>Button>
                        ))}
                                  </Flex>Flex>
                        ...
                        <Container maxW="1200px" py={6}>
                          ...
                          {error && <Alert status="error" mb={4}><AlertIcon />{error}</Alert>Alert>}
                          {!loading && (
                        <>
                          {viewMode === 'dashboard' && (
                                          <Box>
                                            ...
                                            { label: 'Avg Tips/Hr', value: stats ? `$${stats.avgTipsPerHour.toFixed(2)}` : '$0' },
                                                            ...
                                                            <ShiftsByDay shifts={shifts} onEdit={handleEditShift} onDelete={handleDeleteShift} />
                                          </Box>Box>
                                      )}
                          {viewMode === 'byDay' && <ShiftsByDay shifts={shifts} onEdit={handleEditShift} onDelete={handleDeleteShift} />}
                          {viewMode === 'biweekly' && <BiWeeklyHours shifts={shifts} settings={settings} />}
                          {viewMode === 'floor' && <FloorComparison shifts={shifts} />}
                          {viewMode === 'calendar' && <CalendarView shifts={shifts} onDateClick={(date) => { setCalendarDate(date); setViewMode('byDay'); }} />}
                        </>>
                      )}
                        </Container>Container>
